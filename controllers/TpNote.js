import tps from "../models/TpNote.js";
...
export const getTPs = async (req,res,next)=>{
    try{
        const tpss = await tps.findById(req.params.id).populate("idTp").populate("idUser")
...
export const getAllTps = async (req,res,next)=>{
    const failed = true;
    try{
    const desTp = await tps.find().populate("idTp").populate("idUser")
...
export const getAllTpsAp = async (req,res,next)=>{
    ...
    try{
    const desTp = await tps.find({ idCours: { $in: courseIds } }).populate("idTp").populate("idUser")
    res.status(200).json(desTp)
    }
    catch(err){
        next(err);
